...
class ApplicationStatus extends Model
{
    protected $fillable = [
        ...
        'contract_viewed_at',
        'contract_signed_at',
        'cashflows_switch_notice_sent_at',
    ...
    protected $casts = [
        ...
        'cashflows_switch_notice_sent_at' => 'datetime',
    ...
    public function getOrderedTimelineAttribute(): array
    {
        $allSteps = [
            ...
            'cashflows_switch_notice_sent' => ['label' => 'Cashflows Switch Notice Sent', 'timestamp' => $this->cashflows_switch_notice_sent_at, 'progress' => 55],
        ...
        // to this application - unlike the rest, they're not part of every application's
        // journey, so showing them as a pending placeholder before they've even been
        // triggered would be misleading. Once triggered (envelope exists), it belongs in
        // the pending list like any other not-yet-complete step, so it shows as the next
        // thing to do rather than staying invisible until it's fully done.
        $optionalStepsNotYetTriggered = [
            'cashflows_switch_notice_sent' => ! $this->cashflows_docusign_envelope_id,
        ];
        ...
        $pending = collect($allSteps)->filter(
            fn($step, $key) => $step['timestamp'] === null && empty($optionalStepsNotYetTriggered[$key])
        );
    ...
    public function getTimestampsAttribute(): array
    {
        return [
            ...
            'cashflows_switch_notice_sent' => $this->cashflows_switch_notice_sent_at?->format('Y-m-d H:i'),
    ...
    private function getTimestampField(string $step): ?string
    {
        $mapping = [
            ...
            'cashflows_switch_notice_sent' => 'cashflows_switch_notice_sent_at',
